...
| externalClusters | list | `[]` | Externally managed clusters, referenced by declarative `subscriptions` (and by CNPG's replica/import bootstraps). Each entry is passed through verbatim as a CloudNativePG `externalClusters` item, so it accepts the full CNPG schema (connectionParameters, password, sslKey/sslCert/sslRootCert secrets, or a barmanObjectStore). This lets a `standalone` cluster reach an external "publisher" for logical replication. See: https://cloudnative-pg.io/documentation/current/cloudnative-pg.v1/#postgresql-cnpg-io-v1-ExternalCluster |
...
| publications | list | `[]` | Declarative logical replication publications (CNPG `Publication` resources). Each entry publishes a set of changes on a database in this cluster so that a subscriber can consume them, enabling near-zero-downtime migrations (initial sync + ongoing replication) without a second system. See: https://cloudnative-pg.io/documentation/current/logical_replication/#publications |
...
| subscriptions | list | `[]` | Declarative logical replication subscriptions (CNPG `Subscription` resources). Each entry subscribes a database in this cluster to a publication on an external cluster listed in `externalClusters`. Combined with `publications`, this offers a managed, near-zero-downtime migration path: sync, then cut over. See: https://cloudnative-pg.io/documentation/current/logical_replication/#subscriptions |
