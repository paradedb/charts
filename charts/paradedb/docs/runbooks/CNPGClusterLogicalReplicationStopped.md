...
```bash
kubectl logs -n <namespace> pod/<instance-pod-name> --tail=200 | grep -i "subscription\|replication\|worker"
```
...
```bash
kubectl exec -n <namespace> -it services/paradedb-rw -- psql -c "ALTER SUBSCRIPTION <subscription> ENABLE;"
```
