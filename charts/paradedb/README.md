...
The ParadeDB Helm chart supports monitoring via Prometheus and Grafana. To enable monitoring, install the [Prometheus CRDs](https://prometheus-community.github.io/helm-charts) before installing the CloudNativePG operator.

### Installing the CloudNativePG Operator

Skip this step if the CloudNativePG operator is already installed in your cluster. For advanced CloudNativePG configuration and monitoring, please refer to the [CloudNativePG documentation](https://cloudnative-pg.io/documentation/current/).
...
To test changes to the chart on a local Minikube cluster, follow the installation instructions above and replace the `helm upgrade` chart reference with the path to the modified chart directory.
...
Each backup adapter takes its own set of parameters, listed in the configuration table below.
...
> [!WARNING]
> Starting with CloudNativePG 1.26, native backup and recovery capabilities are
> being progressively phased out of the core operator and moved to official
> CNPG-I plugins.

To migrate from the built-in CloudNativePG backup implementation to the Barman
Cloud CNPG-I plugin, change the backup method to `plugin` and specify the plugin
name. Apply the same change to scheduled backups:
...
The chart creates the required `ObjectStore` resource as a Helm hook before
...
| externalClusters | list | `[]` | Externally managed clusters, referenced by declarative `subscriptions` (and by CNPG's replica/import bootstraps) |
...
| publications | list | `[]` | Declarative logical replication publications (CNPG `Publication` resources) |
...
| recovery.method | string | `"backup"` | Available recovery methods: * `backup` - Recovers a CNPG cluster from a CNPG backup (PITR supported) Needs to be on the same cluster in the same namespace. * `plugin` - Recovers a CNPG cluster from a backup taken with a CloudNativePG plugin (e.g. barman-cloud). * `object_store` - Recovers a CNPG cluster from a barman object store (PITR supported). * `pg_basebackup` - Recovers a CNPG cluster via a streaming replication protocol. Useful if you want to        migrate databases to CloudNativePG, even from outside Kubernetes. * `import` - Import one or more databases from an existing Postgres cluster. |
...
| poolers[].type | `PoolerType` | `rw` | Type of service to forward traffic to. Default: `rw`. |
| poolers[].poolMode | `PgBouncerPoolMode` | `session` | The pool mode. Default: `session`. |
| poolers[].authQuerySecret | `LocalObjectReference` | `{}` | The credentials of the user that need to be used for the authentication query. |
...
| poolers[].template | `PodTemplateSpec` | `{}` | The template of the Pod to be created |
| poolers[].serviceTemplate | `ServiceTemplateSpec` | `{}` | Template for the Service the pooler creates |
